...
class qtApp(QMainWindow):
    ...
    def btnDelClicked(self):
        if self.curIdx == 0:
            ...
            reply = QMessageBox.question(self, '확인', '정말로 삭제 하시겠습니까?', QMessageBox.Yes | QMessageBox.No,
                                         QMessageBox.Yes)
    ...
    def btnSaveClicked(self):  # 저장
        ...
        if fullName == '' or phoneNum == '':
            ...
            self.conn = pymysql.connect(host = 'localhost', user = 'root', password = '12345',
                                    db = 'miniproject', charset = 'utf8')
            if self.curIdx == 0: # 신규 네게 변수값 받아서 INSERT 쿼리문 만들기
            ...
            self.initDB()   # QTablWidget에 새 데이터가 출력되도록
    ...
    def makeTable(self, rows):
        ...
        self.tblAddress.setHorizontalHeaderLabels(['번호', '이름', '핸드폰', '이메일', '주소'])  # 열 제목
        self.tblAddress.setColumnWidth(0,0)  # 1. 번호는 숨김
        self.tblAddress.setColumnWidth(1,70)  # 2. 이름열 70
        self.tblAddress.setColumnWidth(2,105)  # 3. 핸드폰 105 
        self.tblAddress.setColumnWidth(3,175)  # 4. 이메일 175 
        self.tblAddress.setColumnWidth(4,200)  # 5. 주소 200
